  ...
  Object.getOwnPropertyNames(proto).forEach((key) => {
    if (key !== 'constructor' && typeof (instance as Record<string, unknown>)[key] === 'function') {
      (instance as Record<string, (...args: unknown[]) => unknown>)[key] = (
        (instance as Record<string, (...args: unknown[]) => unknown>)[key] as (...args: unknown[]) => unknown
      ).bind(instance);
    }
  });
}
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    if (!container) return;
    const app = new GalleryApp(container, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase });
    appRef.current = app;
    ...
    const handleInteractEnd = () => {
      resumeTimerRef.current = setTimeout(() => { userInteractingRef.current = false; }, 2000);
    };
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (!userInteractingRef.current && appRef.current) {
        appRef.current.nextSlide();
      }
    }, autoInterval);
